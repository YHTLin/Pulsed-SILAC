...
lightCDS = score(lightCD, c("^LOG2.*DMSO", "^LOG2.*M1071", "^LOG2.*Rapa"), c("DMSO", "M1071", "Rapa"))
lightCDS = mutate(lightCDS, score_M1071 = slope_M1071 - slope_DMSO)
lightCDS = mutate(lightCDS, score_Rapa = slope_Rapa - slope_DMSO)
heavyCDS = score(heavyCD, c("^LOG2.*DMSO", "^LOG2.*M1071", "^LOG2.*Rapa"), c("DMSO", "M1071", "Rapa"))
  ...
  tabPanel("Heavy",
    ...
    tabsetPanel(
      ...
      tabPanel("Pairwise Plot",
               fluidRow(
                 column(3, br(),  # Vertical spacing
                        p(HTML("Pairwise plots display relevant information on the distribution of
                               log<sub>2</sub>-transformed protein intensities along the diagonal.
                               Additionally, all possible pairwise scatter plots with lines of
                               best fit (red) are shown at the upper right corner. The corresponding
                               correlation coefficients are found at the lower left corner. In some
                               instances, missing values are present and therefore reported. Data
                               are organized by treatment groups.")),
                        hr(),
                        selectInput('pair_plot_heavy_norm', 'Display Type:',
                                    c("LOG2", "NORM"), selectize = FALSE),
                        helpText("Option to view the data prior to (LOG2) or (NORM) following normalization 
      ...
      tabPanel("Heatmap",
               fluidRow(
                 column(3, br(),
                        p(HTML("Heatmaps represent the values in the filtered data table with color.
                        ...
                        selectInput('heatmap_heavy_norm', 'Display Type:',
      ...
      tabPanel("Filtered Table", br(),
               p(HTML("The data table below expresses the pulsed SILAC proteomics output as slopes 
                      ...
                      treatment group from those of DMSO control. Negative scores suggest that the rate of 
                      synthesis decreases with treatment and positive scores indicate the opposite effect. 
           ...
           tabsetPanel(
                      ...
                      fluidRow(
                        column(3, br(),  # Vertical spacing
                               p(HTML("Pairwise plots display relevant information on the distribution of
                                      ...
                                      are organized by treatment groups.")),
                               hr(),
                               selectInput('pair_plot_light_norm', 'Display Type:',
                                           c("LOG2", "NORM"), selectize = FALSE),
                               helpText("Option to view the data prior to (LOG2) or (NORM) following normalization 
                                        to the DMSO 0-hour timepoint.")
                               ),
                      ...
                      fluidRow(
                        column(3, br(),
                               p(HTML("Heatmaps represent the values in the filtered data table with color.
                                      ...
                                      are plotted as a line chart above the figure. See legend below for 
                                      color scale. Missing values are colored in black.")),
                               selectInput('heatmap_light_norm', 'Display Type:',
                                           c("NORM", "LOG2"), selectize = FALSE),
                               helpText("Option to view the data prior to (LOG2) or (NORM) following normalization 
                                        to the DMSO 0-hour timepoint. NORM is recommended.")
                               ),
             ...
             tabPanel("Filtered Table", br(),
                      p(HTML("The data table below expresses the pulsed SILAC proteomics output as slopes 
                             ...
                             to meet this criterion. Scores are computed by subtracting the slopes of the 
                             treatment group from those of DMSO control. Negative scores suggest that the rate of 
                             degradation increases with treatment and positive scores indicate the opposite effect. 
                             <b>All values are extremely sensitive to outliers. Please interpret with caution.</b> 
                             A complete data table including the measured intensities can be downloaded below.")),
...
server = function(input, output) {
  ...
  output$pair_plot_heavy_DMSO <- renderPlot({ 
    if (input$pair_plot_heavy_norm == "LOG2") {
  ...
  output$heatmap_heavy <- renderPlot({ 
    if (input$heatmap_heavy_norm == "LOG2") {
  ...
  output$pair_plot_light_DMSO <- renderPlot({ 
    if (input$pair_plot_light_norm == "LOG2") {
  ...
  output$heatmap_light <- renderPlot({ 
    if (input$heatmap_light_norm == "LOG2") {
